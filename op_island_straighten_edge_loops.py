...
class op(bpy.types.Operator):
    ...
    def poll(cls, context):
        if not context.area or context.area.ui_type != 'UV':
            return False
...
def main(self, context):
    ...
    islands = utilities_uv.getSelectionIslands(bm, uv_layers, extend_selection_to_islands=True,
                                               selected_faces=set(selected_faces_loops.keys()))

    for island in islands:
        selected_loops_island = {loop for face in island.intersection(selected_faces_loops.keys()) for loop in
                                 selected_faces_loops[face]}
...
def get_loops_segments(self, bm, uv_layers, island_loops_dirty):
    ...
    boundary_splitted_edges = {loop.edge for loop in island_loops_dirty if
                               (not loop.edge.is_boundary) and loop[uv_layers].uv.to_tuple(precision) !=
                               loop.link_loop_radial_next.link_loop_next[uv_layers].uv.to_tuple(precision)}
        ...
        def find_next_loop(loop):
            ...
            def get_prev(found_prev):
                if found_prev:
                    for foundLoop in found_prev:
                        if foundLoop[uv_layers].uv.to_tuple(precision) == loop.link_loop_prev[uv_layers].uv.to_tuple(
                                precision):
                            segment.append(foundLoop)
                            for anyLoop in found_prev:
                                if anyLoop[uv_layers].uv.to_tuple(precision) == loop.link_loop_prev[
                                    uv_layers].uv.to_tuple(precision):
            ...
            def get_next(found_next):
                for foundLoop in found_next:
                    if foundLoop[uv_layers].uv.to_tuple(precision) == loop.link_loop_next[uv_layers].uv.to_tuple(
                            precision):
                        segment.append(foundLoop)
                        for anyLoop in found_next:
                            if anyLoop[uv_layers].uv.to_tuple(precision) == loop.link_loop_next[uv_layers].uv.to_tuple(
                                    precision):
                                island_nodal_loops.remove(anyLoop)
                        return foundLoop, False
