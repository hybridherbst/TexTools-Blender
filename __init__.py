...
class Panel_Preferences(AddonPreferences):
    ...
    [
        ...
        ('CUSTOM', 'Custom',
         'Set a global custom RGBA color for the background. Note that a transparent background can be specified')
    ],
    ...
    [
        ...
        ('PBR', 'PBR typical',
         'Set Linear as Color Space for all baked maps except for Diffuse/Base Color, SSS/Emission color, colored Transmission, Environment, Combined or any custom Mode'),
        ('ASTANDARD', 'ACES standard',
         'Set ACES sRGB Texture as Color Space for all baked textures except for Normal maps'),
        ('APBR', 'ACES PBR typical',
         'Set ACES Linear sRGB as Color Space for all baked maps except for Diffuse/Base Color, SSS/Emission color, colored Transmission, Environment, Combined or any custom Mode')
    ],
    ...
    def draw(self, context):
        ...
        if self.bake_device == 'STANDARD':
            ...
            col.label(
                text="Set Linear as Color Space for all baked maps except for Diffuse/Base Color, SSS/Emission color, colored Transmission, Environment, Combined or any custom Mode.")
            ...
            col.label(
                text="Set ACES Linear sRGB as Color Space for all baked maps except for Diffuse/Base Color, SSS/Emission color, colored Transmission, Environment, Combined or any custom Mode.")
        ...
        if not hasattr(bpy.types, "ShaderNodeBevel"):
            ...
            col.operator("wm.url_open", text="Get Blender with Bevel Shader",
                         icon='BLENDER').url = "https://builder.blender.org/download/"
        ...
        col = box.column(align=True)
        col.operator("wm.url_open", text="Donate",
                     icon='HELP').url = "https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=ZC9X4LE7CPQN6"
        col.operator("wm.url_open", text="GIT Code",
                     icon='WORDWRAP_ON').url = "https://github.com/SavMartin/TexTools-Blender"

        col.label(text="Discussions")
        row = col.row(align=True)
        row.operator("wm.url_open", text="BlenderArtists",
                     icon='BLENDER').url = "https://blenderartists.org/forum/showthread.php?443182-TexTools-for-Blender"
        row.operator("wm.url_open",
                     text="Polycount").url = "http://polycount.com/discussion/197226/textools-for-blender"
...
def get_dropdown_uv_values(self, context):
    ...
    if obj and obj.type == 'MESH' and obj.data.uv_layers:
        ...
        for uvLoop in obj.data.uv_layers:
            options.append((str(step), f'UV {step + 1}',
                            f"Change active UV Channel of all selected Objects to '{uvLoop.name}' where possible",
                            step))
            step += 1
    return options
...
class TexToolsSettings(PropertyGroup):
    ...
    UDIMs_source: EnumProperty(items=
                               [('OBJECT', 'From Object',
                                 'Work on the first detected Tiled UDIM Image in the Active Object Materials'),
    ...
    bake_force: EnumProperty(items=
                             [('None', 'Normal', 'Use the default TexTools baking behaviour'),
                              ...
                              ('Multi', 'Multi',
                               'Force a texture bake for each selected Mesh Object by disabling automatic pairing by name')],
                             name="Force", default='None'
                             )
    ...
    bake_color_space: EnumProperty(items=
                                   [('sRGB', 'sRGB', 'Standard RGB output color space for the baked texture'),
                                    ('Non-Color', 'Linear',
                                     'Linear or Non-Color output color space for the baked texture'),
                                    ('Utility - sRGB - Texture', 'Utility - sRGB - Texture',
                                     'ACES Standard RGB Texture output color space for the baked texture'),
                                    ('Utility - Linear - sRGB', 'Utility - Linear - sRGB',
                                     'ACES Linear Standard RGB output color space for the baked texture')],
                                   name="CS",
                                   default='Non-Color',
    ...
    texel_get_mode: EnumProperty(items=
                                 [('IMAGE', 'Image',
                                   'Per object, get the resolution of the first image found in any used material'),
                                  ('SIZE', 'TexTools Size',
                                   'Use the Size specified under the TexTools tab')] + utilities_ui.size_textures,
                                 name="Texture Size",
                                 default='IMAGE'
                                 )
    ...
    [
        ...
        ('9e00af,7026b9,4f44b5,478bf4,39b7d5,229587,47b151,9dcf46,f7f235,f7b824,f95f1e,c5513c,78574a,4d4b4b,9d9d9d',
         '15 Rainbow', '...')
    ],
        description="Color template",
        name="Preset",
    ...
    color_assign_mode: EnumProperty(items=
                                    [('MATERIALS', 'Materials',
                                      'Assign simple colored materials to objects or selected faces'),
                                     ('VERTEXCOLORS', 'Vertex Colors',
                                      'Assign colors as Vertex Colors to objects or selected faces')],
                                    name="Assign Mode",
                                    default='MATERIALS',
            ...
            if obj.type == 'MESH':
                ...
                if not obj.data.uv_layers:
                    ...
                    if image:
                        row = layout.row(align=True)
                        col = row.column()
                        col.template_list("IMAGE_UL_udim_tiles", "", image, "tiles", image.tiles, "active_index",
                                          rows=3, maxrows=3)

        col = layout.column(align=True)
        row = col.row(align=True)
        row.operator(op_texture_reload_all.op.bl_idname, text="Reload Textures",
                     icon_value=icon_get("op_texture_reload_all"))

        if settings.bversion >= 3.0:
            row = col.row(align=True)
            row.scale_y = 1.75
            row.operator(op_texel_checker_map.op.bl_idname, text="Checker Map",
                         icon_value=icon_get("op_texel_checker_map"))
...
class UI_PT_Panel_Layout(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator("wm.url_open", text="",
                         icon='INFO').url = "http://renderhjs.net/textools/blender/index.html#uvlayout"
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        row.operator(op_island_align_world.op.bl_idname, text="Align World",
                     icon_value=icon_get("op_island_align_world"))
        ...
        col.operator(op_align.op.bl_idname, text="↙",
                     icon_value=icon_get("op_align_bottomleft")).direction = "bottomleft"
        ...
        col.operator(op_align.op.bl_idname, text="↘",
                     icon_value=icon_get("op_align_bottomright")).direction = "bottomright"

        row_tr = col_tr.row(align=True)
        col = row_tr.column(align=True)
        col.scale_x = 0.5
        row = col.row(align=True)
        row.operator(op_align.op.bl_idname, text="—",
                     icon_value=icon_get("op_align_horizontal")).direction = "horizontal"
        ...
        col_tr.separator()
        row = col_tr.row(align=True)
        row.operator(op_island_rotate_90.op.bl_idname, text="90° CCW",
                     icon_value=icon_get("op_island_rotate_90_left")).angle = -math.pi / 2
        row.operator(op_island_rotate_90.op.bl_idname, text="90° CW",
                     icon_value=icon_get("op_island_rotate_90_right")).angle = math.pi / 2
        row = col_tr.row(align=True)
        row.operator(op_island_mirror.op.bl_idname, text="Mirror H",
                     icon_value=icon_get("op_island_mirror_H")).is_vertical = False
        row.operator(op_island_mirror.op.bl_idname, text="Mirror V",
                     icon_value=icon_get("op_island_mirror_V")).is_vertical = True

        col = box.column(align=True)
        row = col.row(align=True)
        op = row.operator(op_island_align_sort.op.bl_idname, text="Sort H",
                          icon_value=icon_get("op_island_align_sort_h"))
        op.is_vertical = False
        op.padding = utilities_ui.get_padding()

        op = row.operator(op_island_align_sort.op.bl_idname, text="Sort V",
                          icon_value=icon_get("op_island_align_sort_v"))
        op.is_vertical = True
        op.padding = utilities_ui.get_padding()
        ...
        row = col.row(align=True)
        row.operator(op_island_straighten_edge_loops.op.bl_idname, text="Straight",
        ...
        row = col.row(align=True)
        row.operator(op_select_islands_identical.op.bl_idname, text="Similar",
                     icon_value=icon_get("op_select_islands_identical"))
        row.operator(op_select_islands_overlap.op.bl_idname, text="Overlap",
                     icon_value=icon_get("op_select_islands_overlap"))
        ...
        row.operator(op_select_islands_flipped.op.bl_idname, text="Flipped",
                     icon_value=icon_get('op_select_islands_flipped'))

        if settings.bversion >= 3.2:
            row = col.row(align=True)
            row.operator(op_select_islands_outline.op.bl_idname, text="Bounds",
                         icon_value=icon_get("op_select_islands_outline"))
...
class UI_PT_Panel_Bake(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator("wm.url_open", text="",
                         icon='INFO').url = "http://renderhjs.net/textools/blender/index.html#texturebaking"
    ...
    def draw(self, context):
        ...
        if settings.bversion >= 3.0:
            row = col.row(align=True)
            row.scale_y = 1.5
            row.operator(op_texture_preview.op.bl_idname, text="Preview Texture",
                         icon_value=icon_get("op_texture_preview"))
        ...
        if len(images) > 0:
            ...
            for image in images:
                ...
                row.alignment = 'RIGHT'
                if image.filepath != "":
                    row.operator(op_texture_open.op.bl_idname, text="",
                                 icon_value=icon_get("op_texture_open")).name = image.name
                else:
                    if bpy.app.debug_value != 0:
                        row.operator(op_texture_save.op.bl_idname, text="",
                                     icon_value=icon_get("op_texture_save")).name = image.name
                    else:
                        pass
        ...
        # Bake Mode
        col.template_icon_view(bpy.context.scene, "TT_bake_mode", scale=5.0,
                               scale_popup=preferences.bake_mode_panel_scale)
        ...
        if len(settings.sets) > 0:
            ...
            if count_types['float'] > 0:
                row.operator("uv.op_select_bake_type", text='',
                             icon_value=icon_get("bake_obj_float")).select_type = 'float'

            if count_types['cage'] > 0:
                row.operator("uv.op_select_bake_type", text='',
                             icon_value=icon_get("bake_obj_cage")).select_type = 'cage'
            ...
            countTypes = (0 if count_types['low'] == 0 else 1) + (0 if count_types['high'] == 0 else 1) + (
                0 if count_types['cage'] == 0 else 1) + (0 if count_types['float'] == 0 else 1)
            if countTypes > 2:
        ...
        col = box.column(align=True)
        col.operator(op_bake_organize_names.op.bl_idname, text=f'Organize {len(bpy.context.selected_objects)}x',
                     icon='BOOKMARKS')
...
class UI_MT_op_color_dropdown_convert_from(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(op_color_from_elements.op.bl_idname, text="Mesh Elements",
                        icon_value=icon_get('op_color_from_elements'))
        layout.operator(op_color_from_materials.op.bl_idname, text="Materials",
                        icon_value=icon_get('op_color_from_materials'))
        layout.operator(op_color_from_directions.op.bl_idname, text="Directions",
                        icon_value=icon_get('op_color_from_directions'))
...
class UI_MT_op_color_dropdown_convert_to(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(op_color_convert_texture.op.bl_idname, text="Texture Atlas",
                        icon_value=icon_get('op_color_convert_texture'))
        layout.operator(op_color_convert_vertex_colors.op.bl_idname, text="Vertex Colors",
                        icon_value=icon_get("op_color_convert_vertex_colors"))
...
class UI_PT_Panel_Colors(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator("wm.url_open", text="",
                         icon='INFO').url = "http://renderhjs.net/textools/blender/index.html#colorid"
...
class UI_PT_Panel_MeshTexture(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator("wm.url_open", text="",
                         icon='INFO').url = "http://renderhjs.net/textools/blender/index.html#meshtexture"
    ...
    def draw(self, context):
        ...
        if settings.bversion >= 3.2:
            ...
            row = col.row(align=True)
            row.scale_y = 1.5
            row.operator(op_meshtex_create.op.bl_idname, text="Create UV Mesh",
                         icon_value=icon_get("op_meshtex_create"))
            ...
            if op_meshtex_trim_collapse.is_available():
                row.operator(op_meshtex_trim_collapse.op.bl_idname, text="Collapse Trim",
                             icon_value=icon_get("op_meshtex_trim_collapse"))
        ...
        row = col.row(align=True)
        row.scale_y = 1.5
        row.operator(op_smoothing_uv_islands.op.bl_idname, text="Smooth by UV Islands",
                     icon_value=icon_get("op_smoothing_uv_islands"))
...
def menu_IMAGE_uvs(self, context):
    ...
    layout.operator(op_island_align_world.op.bl_idname, text="Align World",
                    icon_value=icon_get("op_island_align_world"))

    layout.separator()
    layout.operator(op_island_centralize.op.bl_idname, text="Centralize Position",
                    icon_value=icon_get("op_island_centralize"))
...
def menu_IMAGE_select(self, context):
    layout = self.layout
    layout.separator()
    layout.operator(op_select_islands_identical.op.bl_idname, text="Similar",
                    icon_value=icon_get("op_select_islands_identical"))
    layout.operator(op_select_islands_overlap.op.bl_idname, text="Overlap",
                    icon_value=icon_get("op_select_islands_overlap"))
    layout.operator(op_select_zero.op.bl_idname, text="Zero", icon_value=icon_get("op_select_zero"))
    layout.operator(op_select_islands_flipped.op.bl_idname, text="Flipped",
                    icon_value=icon_get('op_select_islands_flipped'))
    if settings.bversion >= 3.2:
        layout.operator(op_select_islands_outline.op.bl_idname, text="Bounds",
                        icon_value=icon_get("op_select_islands_outline"))
...
def menu_IMAGE_MT_image(self, context):
    layout = self.layout
    layout.separator()
    layout.operator(op_texture_reload_all.op.bl_idname, text="Reload Textures",
                    icon_value=icon_get("op_texture_reload_all"))
    if settings.bversion >= 3.0:
        layout.operator(op_texel_checker_map.op.bl_idname, text="Checker Map",
                        icon_value=icon_get("op_texel_checker_map"))
        layout.operator(op_texel_checker_map_cleanup.op.bl_idname, text="Checker Map cleanup", icon='TRASH')
        layout.operator(op_texture_preview.op.bl_idname, text="Preview Texture",
                        icon_value=icon_get("op_texture_preview"))
...
def menu_VIEW3D_MT_object(self, context):
    ...
    if settings.bversion >= 3.0:
        self.layout.operator(op_texel_checker_map.op.bl_idname, text="Checker Map",
                             icon_value=icon_get("op_texel_checker_map"))
        self.layout.operator(op_texel_checker_map_cleanup.op.bl_idname, text="Checker Map cleanup", icon='TRASH')
    if settings.bversion >= 3.2:
        self.layout.operator(op_meshtex_create.op.bl_idname, text="Create UV Mesh",
                             icon_value=icon_get("op_meshtex_create"))
    self.layout.operator(op_smoothing_uv_islands.op.bl_idname, text="Smooth by UV Islands",
                         icon_value=icon_get("op_smoothing_uv_islands"))


def menu_VIEW3D_MT_mesh_add(self, context):
    self.layout.operator(op_meshtex_pattern.op.bl_idname, text="Create Pattern",
                         icon_value=icon_get("op_meshtex_pattern"))
...
def menu_VIEW3D_MT_object_context_menu(self, context):
    ...
    layout.operator(op_smoothing_uv_islands.op.bl_idname, text="Smooth by UV Islands",
                    icon_value=icon_get("op_smoothing_uv_islands"))
...
def register():
    ...
    bpy.types.Object.TT_CM_Scale = bpy.props.FloatProperty(name="Tiling",
                                                           description="Checker Map scale for the Active Object",
                                                           default=1, min=0.0001, max=1000)
...
def unregister():
    ...
    try:
        if hasattr(bpy.types.Object, 'TT_CM_Scale'):
